...
export class BitMazeGame {
  constructor(bytes) {
    if (!bytes) throw new Error("BitMazeGame requires level bytes");
...
export function inspectTile(game, x = game.x, y = game.y) {
  const index = y * game.width + x;
  const byteIndex = Math.floor(index / 8);
  const bitIndex = 7 - (index % 8);
  const mask = 1 << bitIndex;
  const trigger = game.triggerAt(x, y);
  return {
    x,
    y,
    index,
    ...
    mask,
    trigger,
    script: game.level.scripts.get(trigger) ?? null,
    planes: game.level.planes.map((plane) => ({
      byte: plane[byteIndex],
      binary: plane[byteIndex].toString(2).padStart(8, "0"),
      value: (plane[byteIndex] & mask) === 0 ? 0 : 1,
    })),
  };
}

export function decodeGateScript(script) {
  if (!script || script.length !== 6) return null;
  if (script[0] !== 0x10 || script[2] !== 0x10 || script[4] !== 0x32 || script[5] !== 0x01) return null;
  return {
    x: script[1],
    y: script[3],
    text: `PUSH8 ${script[1]}  →  PUSH8 ${script[3]}  →  CLR_WALL  →  HALT`,
  };
}
